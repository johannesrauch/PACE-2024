    ...
    do {                                                                                                              \
        if (info_br.nof_iterations % 50 == 0) {                                                                       \
            fmt::printf("%11s%11s|%11s%11s%11s%11s|%11s%11s%11s|%11s%11s%11s%11s\n", "lb", "ub", "iter", "obj val",   \
                        "nof rows", "nof nodes", "nof bucket", "nof added", "nof delete", "uvw iter", "u", "v", "w"); \
        }                                                                                                             \
        fmt::printf("%11u%11u|%11u%11.0f%11u%11u|%11u%11u%11u|%11u%11u%11u%11u\n", info_br.lower_bound,               \
                    ...
                    info_lp.nof_deleted_rows, info_lp.nof_iterations_3cycles, info_lp.u_old, info_lp.v_old,           \
                    info_lp.w_old);                                                                                   \
        std::cout << std::flush;                                                                                      \
    } while (false)
#else
