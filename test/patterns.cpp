...
void test_pattern_analyzer(const fs::path filepath, const bool test) {
    ...
    pace2024::folded_matrix cr_matrix(graph);
    pace2024::pattern_analyzer pattern(graph, cr_matrix);
    std::vector<T> ordering;
    std::vector<T> positions;
    if (test) {
        pace2024::test::get_ref_ordering(  //
            filepath, graph.get_n_fixed(), graph.get_n_free(), ordering);
        pace2024::inverse(ordering, positions);
    }

    std::vector<std::pair<T, T>> new_fixings;
    for (T u = 0; u < graph.get_n_free(); ++u) {
        for (T v = u + 1; v < graph.get_n_free(); ++v) {
            pace2024::pattern p = pattern.analyze(u, v);
            if (test) {
                if (p == pace2024::pattern::u_before_v) {
                    assert(positions[u] < positions[v]);
                } else if (p == pace2024::pattern::v_before_u) {
                    assert(positions[u] > positions[v]);
                }
            }
            if (p != pace2024::pattern::indeterminate &&  //
                cr_matrix(u, v) > 0 &&                    //
                cr_matrix(v, u) > 0) {
...
int main() {
    ...
    std::cout << "TEST::PACE2024::PATTERNS:\t\t\t\t\tOK" << std::endl;
    return 0;
}
