...
void test_position_oracle(const pace::instance<T, R>& instance, const bool do_test) {
    ...
    pace::digraph restriction_graph(n);
    pace::oracle oracle(instance, ub);

    std::vector<std::pair<T, T>> forced_pairs;
    std::vector<std::pair<T, T>> more_settled_pairs;
    uint32_t d = 0, c = 0, b = 0, p = 0;
    for (T u = 0; u < n; ++u) {
        for (T v = u + 1; v < n; ++v) {
            pace::pattern pattern = oracle.based_on_degree(u, v);
            if (pattern != pace::indeterminate) {
                ++d;
                insert_based_on_pattern(more_settled_pairs, restriction_graph, u, v, pattern);
                continue;
            }
            ...
            // c_uv != c_vu from her
            pattern = oracle.based_on_crossing_numbers(c_uv, c_vu);
            if (pattern != pace::indeterminate) {
                ++c;
                insert_based_on_pattern(forced_pairs, restriction_graph, u, v, pattern);
                continue;
            }
    ...
    const uint32_t nof_settled = d + c + b + p + h;
    fmt::printf("%11s|%11u%11u%11.1f|%11u%11u%11u%11u%11u|%11u%11d%11u\n",  //
                instance.filepath.filename(),                           //
                n_choose_2, nof_settled,
                static_cast<double>(nof_settled) / n_choose_2 * 100,  //
                d, c, b, p, h,                                                   //
